...
| **거버넌스**    | 사후 로그 확인               | PreCompact/Stop hook과 audit-log state-trigger를 운영 계약에 포함 |
...
| Runtime Harness 설계 · canonical event ontology | `mso-harness-setup` |
...
## v0.2.2 변경 이력 — Runtime Harness Toolkit planning

> **provider-free runtime harness 설계 추가.** `mso-harness-setup` 스킬을 추가하여 canonical event ontology, YAML runtime spec, provider adapter, policy engine, runtime evaluator, escalation router의 v0.2.2 planning/spec를 고정한다.
