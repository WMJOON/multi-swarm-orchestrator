# Multi-Swarm Orchestrator (MSO) v0.8.2
...
`mso-hermes-bridge`는 MSO의 기본 실행 plane에서 제거한다. Hermes Agent를 외부 Executor로 붙이는 방식은 스킬 생성과 runtime 배선이 늘어나면서 오류 표면이 커졌고, work-memory/workflow 정리 같은 execution plane 목적에는 LangGraph 기반 artifact가 더 단순하다.

- **라우팅 제거**: `mso-orchestration`에서 Hermes Bridge 트리거와 초기화 안내를 제거한다.
- **설치 제거**: `install.sh`의 `SKILLS` 배열에서 `mso-hermes-bridge`를 제외한다.
- **설정 정리**: 이미 v0.8.0을 적용한 프로젝트는 `python skills/mso-repository-setup/scripts/init.py --cleanup-hermes <project>`로 MSO가 만든 Hermes Bridge 흔적을 정리한다.
- **실행 방향**: execution plane은 `mso-workflow-optimizer`가 TTL workflow를 LangGraph artifact로 컴파일하는 흐름을 우선한다.

상세는 [docs/changelog.md](docs/changelog.md).

### v0.8.0 (2026-07-06) — 폐기됨: Hermes Bridge 외부 Executor 위임

`mso-hermes-bridge`는 v0.8.0에서 추가됐으나 v0.8.1에서 기본 지원을 폐기했다. 이 경로는 재도입하지 않고, 필요한 실행 위임은 LangGraph/provider policy 쪽으로 재설계한다.
...
- **version ladder**: v0.6.x는 workflow shape/observability 강화 패치 계열이고, v0.7.0은 Repository Graph edge-first 온톨로지 재설계, v0.7.1은 UUG 연동 패치, v0.8.0은 Hermes Bridge 실험, v0.8.1은 Hermes Bridge 폐기와 LangGraph execution plane 우선 전환, v0.8.2는 workflow observation alias와 `execution-rail.md` 산출물 분리다.
