# Multi-Swarm Orchestrator (MSO) v0.8.0
...
MSO workflow에서 **Hermes Agent를 외부 Executor로 위임**하는 `mso-hermes-bridge` 스킬을 추가한다. `wf:delegates_to :hermes-executor`로 선언된 step을 Hermes API Server(OpenAI-compatible, port 8642)로 HTTP 위임하고 Runs API 폴링으로 결과를 수집한다.

- **mso-hermes-bridge**: MSO ROADMAP §7 Execution Metadata의 ExecutionMethod=api 구현 사례(ExecutionSubject=Agent, Hermes). `scripts/setup-with-hermes.sh`로 MSO init과 Hermes 세팅을 한 번에 진행하거나, `hooks/hermes-repo-setup.sh`로 기존 MSO repository에 Hermes만 추가한다.
- **mso-orchestration 배선**: Hermes 위임 라우팅 트리거(`hermes 위임`, `hermes-bridge`, `외부 에이전트 위임`, `delegates_to hermes`)를 SKILL.md에 등록.
...
- **version ladder**: v0.6.x는 workflow shape/observability 강화 패치 계열이고, v0.7.0은 Repository Graph edge-first 온톨로지 재설계, v0.7.1은 UUG 연동 패치, v0.8.0은 Hermes Bridge 외부 executor 위임 스킬 추가다.
