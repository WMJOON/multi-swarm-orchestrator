...
- workflow의 SSOT는 `workflow/*.abox.ttl`이다. YAML은 legacy/edit layer이고 `migrate_workflows_to_ttl.py`로 TTL 정본을 생성·검증한다.
...
workflow/*.yaml               →   legacy/edit layer
  ...
  + migrate_workflows_to_ttl.py    기존 YAML → TTL ABox 정본 마이그레이션
...
`index.yaml`과 workflow TTL ABox는 선언으로 끝나지 않는다. Claude Code hook이 세션 중 도구 사용과 세션 종료를 감지하여 실행 컨텍스트를 `work-memory`에 자동으로 적재한다.
...
workflow TTL ABox의 세 가지 노드 타입이 에이전트의 실행 경로를 컴파일 타임에 제약한다. YAML은 legacy/edit layer로만 둔다.
