...
이 스킬은 **구조적 검증**과 **decision subject taxonomy** 만 강제한다. 네이밍 패턴·동사 어휘·phase 이름·directory role 어휘 등은 **프로젝트 영역**.
...
| `branches.on` routing case 존재 | `phase.id` 어휘 (discovery/development/testing …) |
...
| 필수 필드 | `decision_subject`, `branches[].on` | `harness`, `pass_criteria` | `oracle_type`, `criteria` |
| 역할 | process branch subject | script/test runner | user/agent/metric eval gate |
...
| **TBox** (타입, 생성물) | [references/tbox/workflow-tbox.ttl](references/tbox/workflow-tbox.ttl) | 클래스 계층(Phase/Node⊃Step·Decision·Validation·Group, WorkflowRef/Module/Milestone) + property domain/range + 통제어휘(decision subject skos). |
...
| 검사 | 엔진 | 잡는 것 |
|------|------|--------|
| **로컬/정합 shape** | pyshacl (추론 off) | status·decision_subject enum, validation=harness+pass_criteria, decision=decision_subject, label 존재, dependsOn/hasNode/next/gotoNode/criticalDep/milestoneOf **range-class**(dangling ref 포함) |
...
| `phase.schema.yaml` | phase | 필수 필드, status enum, default_decision_subject, `workflows[]` (sub workflow ref) |
