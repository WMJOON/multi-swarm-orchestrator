...
Eval 노드는 반드시 평가 대상 workflow(`wf:target`), 측정 대상 artifact(`wf:targetArtifact`), `on=fail` 개선 task, `on=pass` terminal branch를 선언해야 한다. 결과 report artifact가 있으면 `wf:orderArtifact`로 별도 선언한다.
...
| **evolves** | downstream `wf:Task.wf:evolves` | Eval 결과를 반영해 개선되는 대상 `wf:Workflow`이며, 해당 Eval의 `wf:target`과 일치해야 함 |
...
스케줄러·webhook·수동 실행 같은 workflow 진입점은 `wf:Event`로 TTL ABox에 명시한다. 관측기는 `.github/workflows`나 `index.yaml`을 보고 스케줄러를 추론하지 않고, 선언된 `wf:Event`를 그대로 렌더링한다.

```ttl
<wf:node/my-e-001> a wf:Event, wf:Node ;
    wf:label "정기 실행 스케줄러" ;
    wf:eventKind "scheduler" ;
    wf:source ".github/workflows/weekly.yml" ;
    wf:status "active" ;
    wf:next <wf:node/my-s-001> .
```

`Event`는 실행 `Task`가 아니므로 `wf:Task` 타입을 붙이지 않는다.

#### Decision Node 필수 구조

`wf:Decision` 노드는 `wf:hasBranch`를 **2개 이상** 선언해야 한다. 단일 경로라면 `step`으로 대체한다. `validate_abox.py`가 강제한다. branch target과 같은 `wf:next`를 동시에 두면 관측 그래프에서 `on:<case>`와 `next`가 같은 방향으로 중복되므로 금지한다.
...
- 레이어는 노드 type 이 아니라 **edge** 로 구분한다.
  ...
  - **oracle(대상)**: `exercises`(평가 목적 실행, 대상 불변), `evolves`(개선). `evolves`는 Eval 이후 Task가 대상 workflow를 개선한다는 edge다.
  - **경계**: `measured_by`(artifact/result→eval)
...
phases:
  - id: ph
    steps:
      - type: eval
        id: e-001
        target_artifact: generated/output.md
        oracle_type: metric
        criteria: ["quality gate passed"]
      - type: step
        id: s-002
