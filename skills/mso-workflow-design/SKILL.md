...
description: >
  ...
  허용한다.
  MOTIF 패턴(Workflow/Step/Decision/Eval/Deliverable/Dependencies/Success Criteria/
...
> 프로젝트 안에서 **root-workflow**(프로젝트 루트, 모듈 조율) + **sub-workflow**(모듈·lifecycle 단위)
...
> 키를 **workflow 가 아닌 확장으로 무시**한다 → 한 워크플로가 MSO 구조 검증을 통과하면서 소비자
> 실행 메타(예: MSM `x_msm`: inputs/outputs/runtime/governance)를 함께 실을 수 있다.

## Abstraction Principle

이 스킬은 **구조적 검증**과 **decision subject taxonomy** 만 강제한다. 네이밍 패턴·동사 어휘·workflow 이름·directory role 어휘 등은 **프로젝트 영역**.

| 스킬이 강제하는 것 | 프로젝트가 정의하는 것 |
|----------------|------------------|
| `type ∈ {step, decision, eval, group}` | `id` 네이밍 패턴 (예: `{abbr}-s-{NNN}`) |
| 노드 id unique, label 존재 | 모듈 약어 매핑 (acp/sdm/…) |
| `decision_subject ∈ {user, agent}` | label 동사 어휘 (분석/생성/검증 …) |
| `branches.on` routing case 존재 | workflow id 어휘 (discovery/development/testing …) |
| agent decision → branch case/criteria 명시 | `directories.role` 어휘 (input/output/reference/…) |
...
|------|-------------------------|------------------|
| **Agentic Workflow** | `wf:Workflow`, `wf:Step`, `wf:Decision`, `wf:Eval`, `wf:next`, `wf:hasBranch` | 어떤 agentic task가 실행되고, 어떤 순서·분기·평가로 진행되는가? |
...
5. work-memory의 `IN/TS/AD/UD/EP/PT/PR` entry는 workflow 업데이트 evidence로 사용한다. 반복 실패·결정 drift·artifact consumer 누락이 보이면 Markdown graph가 아니라 TTL ABox를 수정하고 관측 뷰를 재생성한다.
...
| loop 통제 | 단독으로 drift loop를 끊지 않음 | feedback loop의 개입점 |
...
여러 후보 artifact 중 하나를 고르거나 재생산 여부를 판단하는 경우는 `Eval`이 아니라 `decision_subject=user`인 `Decision`이다. `Eval`은 NLU engine process, evaluator, corpus 품질, 배포 가능성처럼 대상 artifact/process 자체의 품질·정합성을 판정할 때만 사용한다. 이때 `[[process]]`/`[[processing artifact]]`는 workflow control node가 아니라 tool use를 가리키는 artifact-like target으로 다룬다. 관측 그래프에서는 `Eval --target--> [[tool]]`, `tool-produced artifact --validated_by--> Eval`, `Eval --requests_revision--> remediation agentTask`, `Eval --approves--> next agentTask/end`, `remediation agentTask --target--> [[tool]]` 형태로 표현한다. agentTask가 실행 도구를 위임할 때만 `agentTask --delegates_to--> [[tool]]` 를 별도 표시한다.
...
| 방향 | TTL 프로퍼티 | 의미 |
|------|------------|------|
| **target** | `wf:Eval.wf:targetArtifact`가 tool일 때 | Eval이 평가 대상으로 삼는 tool/process |
| **target** | `wf:Step.wf:targetArtifact`가 tool일 때 | Eval이 요청한 개선 task가 수정·조정 대상으로 삼는 tool/process |
| **validated_by** | `wf:Eval.wf:targetArtifact` 또는 target tool의 produced artifact | artifact가 어떤 Eval에 의해 검증되는지 표시 |
| **requests_revision** | `wf:Eval.wf:orderTarget` | Eval이 개선·재작업을 요청하는 downstream remediation/optimization agentTask |
| **approves** | `wf:Eval.wf:next`, 없으면 end boundary | Eval 통과/확정 시 승인 진행되는 agentTask/end |
...
<wf:node/my-d-001> a wf:Eval, wf:Node ;
    ...
    wf:targetArtifact "data/labeling.db#labels" ; # DB table artifact
    wf:orderTarget "my-s-007" ;                 # requests_revision 수신 step
...
각 workflow scope는 workflow/node URI에 포함한다. 예: `wf:workflow/lifecycle/discovery`, `wf:node/lifecycle/lc-s-010`.
...
대형 워크플로우는 root → sub workflow 로 분리할 수 있다. lifecycle 단계도 sub workflow 이며, 계층은 `wf:has_subWorkflow` 하나로 표현한다.

TTL ABox에서는 `wf:has_subWorkflow` 직접 edge로 계층 참조를 표현한다. legacy YAML의 `phase.workflows[]`는 migration compatibility로 읽되 정본 생성물에는 `wf:WorkflowRef`를 만들지 않는다.

**규칙**:
- `ref` 는 legacy migration provenance 또는 외부 sub workflow 파일 참조로 남길 수 있다.
- `module` 이 있으면 scaffold `module:{module}.path` 와 sub 파일의 `module.id`를 cross-check 한다.
- `ref` 의 anchor (`#...`) 는 sub 파일의 workflow/group id 와 일치해야 함.
...
- [references/gate-levels.md](references/gate-levels.md) — user/agent decision subject와 파생 intervention level 분류
- [references/workflow-patterns.md](references/workflow-patterns.md) — Workflow Pattern (Local File Versioning, Git Versioning, Work-Memory Feedback Update)

> **MOTIF vs Pattern**: MOTIF는 `id`, `decision_subject` 같은 **필드 구조**가 고정된 단위.
...
| **TBox** (타입, 생성물) | [references/tbox/workflow-tbox.ttl](references/tbox/workflow-tbox.ttl) | 클래스 계층(Workflow/Node⊃Step·Decision·Eval·Group, Module/Milestone) + property domain/range + 통제어휘(decision subject skos). |
...
| **ABox** (인스턴스, **SSOT-of-record**) | 프로젝트별 workflow (`a wf:Workflow` …) `*.abox.ttl` | 실제 워크플로 정본. 신규 변경은 TTL에서 직접 수행한다. **커밋 대상은 이 파일.** |

> **schemas = SSOT, TBox/SHACL = 생성물**: `schemas/*.yaml` 의 `required`/`type`/`enum`/
> `required_when` 등을 `scripts/schemas_to_tbox.py` 가 TBox+SHACL 로
...
> ```
> 생성 못 하는 것(산문 invariant, branch.on routing case 의미, 교차-스킬)은
> wf_to_ttl.py SPARQL + 수작업으로 분리 유지. schema 없는 root 개념(Module/
> Milestone/directory/workflow hierarchy)은 생성기 내 `_GRAPH_OVERLAY` 에 명시.
...
| **Feedback loop control** | SHACL-SPARQL + rdflib SPARQL | `wf:next`/`wf:gotoNode` 순환 중 별도 `wf:Eval` gate가 없는 loop를 오류로 판정. |
...
**자기-검증 배선**: legacy YAML migration 단계에서는 metric Eval 노드에서 `harness: wf_shape_validator`
...
**Workflow 구조 (스킬이 검증)**
- [ ] 각 노드에 `type` ∈ {step, decision, eval, group} 명시
...
- [ ] 각 metric eval 에 `harness` (runner 식별자), `criteria` 명시
- [ ] `branches[].on` 이 stable routing case 로 명시됨
- [ ] workflow 에 `status` ∈ {completed, active, pending}

**계층 참조 (스킬이 검증)**
- [ ] `workflow.workflows[]` 항목에 `ref` 명시
- [ ] `ref` 의 file path 가 scaffold module path 의 자손 (cross-skill, `--scaffold` 옵션으로 확인)
- [ ] `ref` 의 anchor (`#...`) 가 sub 파일의 workflow/group id 와 일치
- [ ] 계층 depth ≤ 3, 순환 참조 없음
- [ ] 동일 harness runner id 가 여러 eval 노드에 등장 시 의도 확인 (WARN)

**프로젝트 컨벤션 (프로젝트가 정의·검증)**
- [ ] `id` 네이밍 패턴 (예: `{module-abbr}-s-{NNN}`)
- [ ] label 동사 어휘 일관성
- [ ] workflow id 어휘 (discovery/development/testing 등)
- [ ] `directories.role` 어휘 (input/output/reference/instruction 등)
- [ ] `success_criteria` 강제 workflow·개수
...
| `decision.schema.yaml` | `type: decision` | decision_subject(user/agent), routing case, `description` |
