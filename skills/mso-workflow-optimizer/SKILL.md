...
### mso-model-optimizer 연동 (v0.1.0)

> *Automation Escalation and Label-Lean Training, Progressively*

Tier Escalation이 발생하면 `mso-model-optimizer`에 Handoff Payload를 전달하여 경량 모델 생성을 트리거한다. v0.1.0부터 라벨이 부족한 환경에서도 Label Strategy(LS-0~3)가 자동으로 라벨 확보 전략을 선택하고, TL-20은 SetFit/LoRA/QLoRA/표준 FT 중 최적 경로를 라우팅한다.

| Escalation 이벤트 | model-optimizer 흐름 | Handoff Payload |
|-------------------|---------------------|-----------------|
| Level 30 → Level 20 | Phase 1.5(Label Strategy) → Phase 2(TL-20: SetFit/LoRA/표준 FT) | `escalation.from_level=30, to_level=20` |
| Level 20 → Level 10 | Phase 2(TL-10: Rule/Heuristic) | `escalation.from_level=20, to_level=10` |
...
4. model-optimizer가 Phase 1(데이터 수집) → Phase 1.5(Label Strategy) → Phase 2(TL 판단) 순서로 진행
