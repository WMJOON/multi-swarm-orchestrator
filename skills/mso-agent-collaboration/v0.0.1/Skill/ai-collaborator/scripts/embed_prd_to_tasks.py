...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="-",
        help="Input tasks.json path. Use '-' (default) to read from stdin.",
    )
    ...
    if args.inplace and args.tasks_file == "-":
        raise SystemExit("--inplace cannot be used with stdin input ('-').")

    tasks_path = None if args.tasks_file == "-" else Path(args.tasks_file)
    ...
    output_data = {"tasks": tasks} if wrapped else tasks

    if args.inplace:
        write_json(Path(args.tasks_file), output_data)
        return
    if args.out:
        write_json(Path(args.out), output_data)
        return

    sys.stdout.write(json.dumps(output_data, indent=2, ensure_ascii=False) + "\n")
