...
> 자동 넛지: `work-memory-check.sh` 가 Stop(매 턴, 커밋 기반 — `additionalContext` 비차단 주입)·SessionStart(compact/resume — 세션 회고)에서 위 판단을 상기시킨다. 훅 넛지는 백스톱이고, 주 레버는 이 always-on 텍스트다.
