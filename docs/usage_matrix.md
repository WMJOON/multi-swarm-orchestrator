...
| mso-mental-model-design | `single` | 설계 | Vertex Registry: directive 택소노미 관리·검색·바인딩 |
...
| mso-agent-collaboration | `single`(run), `parallel`(batch/swarm) | 실행 | 티켓 Dispatch + 멀티 프로바이더 CLI 실행 (Codex·Claude·Gemini). `collaborate.py`로 run·batch·swarm 모드 직접 실행 가능 |
| mso-harness-setup | `single` | 런타임 설계 | Runtime Harness Toolkit planning: canonical event ontology, YAML runtime spec, provider adapter, policy/evaluator/escalation 계약 |
...
| mso-model-optimizer | `single`, `loop`(재학습 포함) | 운영 | Smart Tool 경량 모델 학습·평가·배포 → deploy_spec + model artifact 생성 |
...
| mso-harness-setup | ✅(harness config) | ✅(runtime event) | ⚪ | ⚪ | ⚪ | ✅(policy/evaluator) | ✅(escalation) |
...
- **Runtime Harness 설계**: `mso-harness-setup` → `mso-task-execution`/`mso-agent-audit-log`/`mso-observability` 연결면 반영
...
sequenceDiagram
    ...
    Ops->>Ops: mso-harness-setup<br/>[Runtime Harness Contract]
    ...
    Ops->>Ops: mso-model-optimizer → mso-task-context-management<br/>[CC-13: deploy_spec → 배포 티켓]
