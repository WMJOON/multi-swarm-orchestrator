...
def _stable_task_suffix(provider: str, prompt: str, model: Optional[str]) -> str:
    seed = f"{provider}|{model or ''}|{prompt}"
    return hashlib.sha1(seed.encode("utf-8")).hexdigest()[:8]


def _parse_provider_model(value: str) -> Tuple[str, Optional[str]]:
    if "@" not in value:
    ...
    model = model.strip()
    if not provider:
        raise ValueError("Provider is empty in task spec.")
    if not model:
        raise ValueError("Model is empty after '@' in task spec.")
...
def parse_task_spec(spec: str, allowed_providers: List[str]) -> TaskRequest:
    parts = spec.split(":", 2)
    if len(parts) < 2:
        raise ValueError(f"Invalid task spec: {spec}. Format: provider[@model]:prompt[:id]")

    provider, model = _parse_provider_model(parts[0])
    prompt = parts[1].strip()
    task_id = (
        parts[2].strip()
        if len(parts) > 2 and parts[2].strip()
        else f"{provider}_{_stable_task_suffix(provider, prompt, model)}"
    )
