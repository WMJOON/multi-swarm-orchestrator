...
| 개념                    | 정의                                                                                                                       |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **apo**                 | Automatic Prompt Optimization. LLM-as-a-judge 라벨과 인간 라벨의 불일치를 분석해 라벨링 프롬프트를 자동 개선하는 수렴 루프 |
...
### Phase 1.6: APO — LLM 라벨링 프롬프트 최적화 (조건부)

> HITL 검수 결과가 일정 수 이상 누적되어 인간 라벨과 LLM-as-a-judge 라벨의 불일치율(`disagreement_rate`)이 임계값을 초과할 때만 실행.
> 기본 임계값: `disagreement_rate ≥ 0.15` (15%). 미달 시 건너뜀.

**목적**: LLM-as-a-judge가 인간 판단과 수렴하도록 라벨링 프롬프트를 점진적으로 개선한다.

#### Step 1 — 불일치 신호 계산

| 신호                        | 계산 방법                                                |
| --------------------------- | -------------------------------------------------------- |
| `disagreement_rate`         | `(llm_label ≠ human_label)` / 전체 HITL 검수 건수        |
| `confusion_score` per class | `저신뢰 비율 × 0.4 + 불일치율 × 0.4 + 저마진 비율 × 0.2` |
| `safety_gate`               | 4개 조건 전체 통과 여부 (아래 참조)                      |

**Safety Gate (4조건)**:

| 조건               | 기준                                |
| ------------------ | ----------------------------------- |
| `delta_ok`         | `mean_confidence ≥ baseline − 5%p`  |
| `worst_p10_ok`     | 하위 10% 최솟값 `≥ baseline − 10%p` |
| `deterioration_ok` | 저신뢰 샘플 비율 `≤ 5%`             |
| `disagreement_ok`  | 불일치율 `≤ 이전 라운드 + 5%p`      |

Safety Gate 전체 통과 → APO 불필요 → 건너뜀.
1개 이상 실패 → Step 2 진행.

#### Step 2 — 혼동 패턴 추출 + Agent Discussion

`confusion_score` 기준 상위 N개 클래스(기본 10개)를 대상으로 구조화된 인터뷰를 진행한다.
msm-mece-validator의 Calibrated Validation 루프와 동일한 방식으로, 조기 종료 조건을 두어 불필요한 반복을 방지한다.

**인터뷰 질문 순서**:
1. 혼동이 잦은 클래스 쌍을 제시 → "경계 조건이 명확한가? 반례 예시가 있는가?"
2. 현재 프롬프트의 해당 클래스 정의를 노출 → "누락된 제약이 있는가?"
3. 확정된 운영 정책(`human_decision_records`) 로드 → LLM이 위반하면 안 되는 제약으로 고정

**조기 종료 조건**:
- 동일 weakest 클래스가 2라운드 연속 반복 → 루프 탈출
- 사용자가 "충분하다" 응답 → 즉시 종료

#### Step 3 — 프롬프트 초안 생성

수집된 혼동 패턴 + 운영 정책 제약을 LLM에 전달하여 개선된 프롬프트 초안을 생성한다.

**생성 규칙**:
- 원본 구조(`{intent_list}`, `{text}`, `{context}` 등 변수)는 반드시 유지
- 혼동 클래스에 명확한 정의·경계 조건·반례 추가
- 기존 잘 동작하는 부분은 수정하지 않음
- 초안은 `is_active: false` 상태로 저장 — 사람 승인 전 미적용

#### Step 4 — HITL 검토 + 활성화

1. 초안 프롬프트와 변경 요약을 사용자에게 제시
2. 사용자 결정:
   - `accept` → 프롬프트 활성화 + `apo_decision` 기록
   - `reject` → 현재 프롬프트 유지 + 거절 사유 기록
   - `needs_revision` → 수정 요청 → Step 3 재실행

**산출물**: `apo_result { version_id, disagreement_before, recommended_decision, activated: bool }`
...
## Operational Module: apo-prompt-optimization

라운드 완료 후 `disagreement_rate`를 지속 추적하여 LLM-as-a-judge 프롬프트를 자동으로 개선한다. Phase 1.6을 독립적으로 반복 실행하는 루프다.

**트리거 조건**: HITL 검수 건수 누적 ≥ 50건 AND `disagreement_rate ≥ 0.15`

**수렴 판정**: 연속 2라운드에서 `disagreement_rate < 0.10` → APO 루프 종료, 프롬프트 고정

**apo_observability 기록 항목**:
...
```json
{
  ...
  "recommended_decision": "accept | reject | needs_revision",
  "activated": true,
  "apo_decision_log": "<path>"
}
```

상세: [modules/module.apo-prompt-optimization.md](modules/module.apo-prompt-optimization.md)
...
| APO 프롬프트 최적화 루프 (Phase 1.6) | [modules/module.apo-prompt-optimization.md](modules/module.apo-prompt-optimization.md) |
